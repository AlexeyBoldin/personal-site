...
function updateAverageRating(approvedReviews) {
	...
	// "Положительный" отзыв — 4 или 5 звёзд
	if (positiveEl) {
		const positiveCount = approvedReviews.filter(r => Number(r.rating) >= 4).length;
		const percent = Math.round((positiveCount / count) * 100);
	...
	if (starsEl) {
		...
		const roundsUpToFull = clamped - fullStars >= 0.75;
		const effectiveFull = fullStars + (roundsUpToFull ? 1 : 0);
		const emptyStars = 5 - effectiveFull - (hasHalf ? 1 : 0);

		let html = "";
		for (let i = 0; i < effectiveFull; i++) html += '<i class="fa-solid fa-star"></i>';
