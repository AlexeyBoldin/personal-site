...
// Тарифы скидки за количество. Пороги фиксированы в коде (не редактируются
// из админки) — только сам механизм можно включить/выключить галочкой
// "Скидки за количество активны".
const BULK_DISCOUNT_TIERS = [
	{ min: 2, max: 4, percent: 5 },
	{ min: 5, max: 9, percent: 10 },
	{ min: 10, max: 24, percent: 15 },
	{ min: 25, max: 49, percent: 20 },
	{ min: 50, max: 99, percent: 25 }
];

function getBulkDiscount(qty) {
	if (qty >= 100) return { percent: 0, individual: true };
	const tier = BULK_DISCOUNT_TIERS.find(t => qty >= t.min && qty <= t.max);
	return { percent: tier ? tier.percent : 0, individual: false };
}
...
function updatePrice() {
	...
	// Не даём ввести больше, чем реально есть на складе
	...
	if (currentProduct && currentProduct.bulk_discount_enabled) {
		const { percent, individual } = getBulkDiscount(qty);

		if (individual) {
			if (discountLineEl) {
				discountLineEl.textContent = "🎉 Для заказов от 100 шт. скидка обсуждается индивидуально — свяжитесь с нами";
				discountLineEl.hidden = false;
			}
		} else if (percent > 0) {
			unitPrice = productPrice * (1 - percent / 100);
			if (discountLineEl) {
				discountLineEl.textContent = `🎉 Скидка ${percent}% за количество (от ${qty} шт.)`;
				discountLineEl.hidden = false;
			}
