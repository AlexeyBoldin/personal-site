...
async function loadApprovedReviews() {
	...
	try {
		...
		if (data.length === 0) {
			...
			empty.className = "no-reviews-yet";
			empty.textContent = "Пока нет отзывов — станьте первым!";
...
function updateAverageRating(approvedReviews) {
	...
	const positiveEl = document.getElementById("positiveReviewsPercent");

	const count = approvedReviews.length;

	if (count === 0) {
		// Нет ни одного одобренного отзыва — оставляем заглушки из вёрстки,
		// просто уточняем подпись у среднего рейтинга
		if (valueEl && labelEl) labelEl.textContent = "Пока нет отзывов";
		return;
	}

	// Процент напрямую из среднего балла (4.2/5 → 84%) — так оба числа
	// на странице логически согласованы друг с другом
	if (positiveEl) {
		const sumForPercent = approvedReviews.reduce((acc, r) => acc + (Number(r.rating) || 0), 0);
		const avgForPercent = sumForPercent / count;
		const percent = Math.round((avgForPercent / 5) * 100);
		positiveEl.textContent = `${percent}%`;
	}
	...
	if (starsEl) {
		const percentFilled = Math.max(0, Math.min(100, (average / 5) * 100));

		starsEl.innerHTML = `
			<span class="star-fill-wrap">
				<span class="star-fill-bg">${'<i class="fa-solid fa-star"></i>'.repeat(5)}</span>
				<span class="star-fill-fg" style="width:${percentFilled}%">${'<i class="fa-solid fa-star"></i>'.repeat(5)}</span>
			</span>
		`;
	}
...
(function () {
	...
	function showInput(placeholder, mode) {
		...
		// Цифровая клавиатура на мобильном для телефона и номера заказа
		if (mode === "phone") {
			input.inputMode = "tel";
			input.value = "";
		} else if (mode === "order") {
			input.inputMode = "numeric";
			input.value = "SW-"; // буквы префикса подставляем сами — пользователь вводит только цифры
		} else {
	...
	// Маска номера заказа: буквы SW- подставлены сами, пользователь вводит
	// только цифры — дефис перед последними 4 цифрами появляется сам
	function maskOrderNumber(rawValue) {
		const digits = rawValue.replace(/\D/g, "").slice(0, 12);
		const datePart = digits.slice(0, 8);
		const randPart = digits.slice(8, 12);

		let result = "SW-";
		result += datePart;
		if (randPart) result += "-" + randPart;

		return result;
	}

	// Маска телефона / номера заказа — только пока реально спрашиваем то или другое
	input.addEventListener("input", function () {
		if (this.dataset.mode === "phone") {
			this.value = maskPhoneValue(this.value);
		} else if (this.dataset.mode === "order") {
			this.value = maskOrderNumber(this.value);
		}
	});
	...
	const CHAT_STORAGE_KEY = "sparkwatch_chat_conversation";

	let conversationId = null;
	let clientToken = null;
	let pendingCustomerName = "";
	let pollTimer = null;
	let waitingForReplyTimer = null;
	const knownMessageIds = new Set();

	function saveConversationToStorage() {
		try {
			localStorage.setItem(CHAT_STORAGE_KEY, JSON.stringify({ conversationId, clientToken }));
		} catch (e) {
	...
	function loadConversationFromStorage() {
		try {
			...
			if (parsed && parsed.conversationId && parsed.clientToken) {
				conversationId = parsed.conversationId;
				clientToken = parsed.clientToken;
			}
		} catch (e) {
			// повреждённые данные — просто начинаем новый диалог
		}
	}
	...
	function startSellerContact() {
		...
		// Разговор уже начат (в этой сессии или восстановлен из localStorage)
	...
	async function sendCustomerMessage(text) {
		try {
			...
			// Если оператор не ответит за отведённое время — предупредим покупателя
			clearTimeout(waitingForReplyTimer);
			waitingForReplyTimer = setTimeout(() => {
				addMessage(CHAT_KNOWLEDGE.operatorsBusy, "bot");
			}, 90000);
	...
	async function fetchNewMessages() {
		...
		try {
			...
			(data || []).forEach(m => {
				...
				// Раз ответ пришёл — таймер "операторы заняты" больше не нужен
				clearTimeout(waitingForReplyTimer);
			});
