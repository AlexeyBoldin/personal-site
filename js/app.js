...
(function () {
	...
	function goToMenu() {
		...
		hideInput();
		setQuickReplies([
			{ label: "❓ Частые вопросы", action: showFaqList },
			{ label: "📦 Статус заказа", action: startOrderTracking },
			{ label: "📞 Связаться с человеком", action: showContacts }
		]);
	}
		...
		const options = faqBase.map(item => ({
			...
			action: () => {
				...
				setQuickReplies([
					...
					{ label: "🏠 В меню", action: () => { addMessage("В меню", "user"); goToMenu(); } }
				]);
			}
		}));

		options.push({ label: "🏠 В меню", action: () => { addMessage("В меню", "user"); goToMenu(); } });
	...
	function startOrderTracking() {
		...
		pendingOrderNumber = "";
		setQuickReplies([
			{ label: "🏠 Отмена, в меню", action: () => { addMessage("Отмена", "user"); goToMenu(); } }
		]);
		addMessage("Введите номер вашего заказа (например, SW-20260721-1234):", "bot");
		showInput("Номер заказа");
	}
	...
	async function lookupOrder(orderNumber, phone) {
		try {
			...
			if (!data || data.length === 0) {
				addMessage("Не нашёл заказ с таким номером и телефоном. Проверьте данные и попробуйте ещё раз, либо свяжитесь с поддержкой.", "bot");
				goToMenu();
				return;
			}
			...
			addMessage("Не удалось проверить статус заказа. Попробуйте позже или напишите в поддержку.", "bot");
			goToMenu();
		}
	}
	...
	inputForm.addEventListener("submit", e => {
		...
		if (state === "awaiting_order_number") {
			...
			addMessage("Спасибо! Теперь введите телефон, который указывали при заказе:", "bot");
			showInput("Телефон");
			return;
		}
	...
	toggleBtn.addEventListener("click", () => {
		...
		if (!panel.hidden && !opened) {
			opened = true;
			addMessage("Здравствуйте! 👋 Я помогу найти ответ на частый вопрос или проверить статус заказа.", "bot");
			goToMenu();
		}
	});
