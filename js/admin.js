...
function renderConversationList() {
    ...
    chatConversations.forEach(conv => {
        ...
        item.innerHTML = `
            <strong>${escapeHtml(conv.customer_name) || "Без имени"}</strong>
...
async function openConversation(id) {
    ...
    if (conv.order_number) {
        const { data } = await supabaseClient
            ...
            .maybeSingle();
        if (data) orderStatus = data.status || "—";
    }
    ...
    detailEl.innerHTML = `
        <div class="chat-customer-card">
            ...
            <span>🚚 ${escapeHtml(orderStatus)}</span>
        </div>
    ...
    (messages || []).forEach(m => threadEl.appendChild(buildAdminMsgEl(m)));
...
// Мгновенно показываем новые сообщения покупателя оператору, без обновления страницы
supabaseClient
    ...
    .on(
        ...
        payload => {
            ...
            if (payload.new.conversation_id === activeConversationId) {
                const threadEl = document.getElementById("chatThread");
                if (threadEl) {
                    threadEl.appendChild(buildAdminMsgEl(payload.new));
                    threadEl.scrollTop = threadEl.scrollHeight;
                }
            }
        }
